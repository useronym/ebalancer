...
%% Search functionality added by Adam 'entity', 'osense' Krupicka
%% 
...
%% Search API
keydelete(Key, N, {In, []}) when is_list(In) ->
    Out = lists:reverse(In),
    keydelete(Key, N, {[], Out});
keydelete(Key, N, {In, Out}) when is_list(In), is_list(Out) ->
    case lists:keytake(Key, N, Out) of
        false ->
            case lists:keytake(Key, N, In) of
                false ->
                    {In, Out};
                {_, _, NewIn} ->
                    {NewIn, Out}
